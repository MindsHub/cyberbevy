...
impl Plugin for SettingsPlugin {
    fn build(&self, app: &mut App) {
        app
            .init_state::<SettingState>()
            .init_resource::<Resolution>()
            .add_systems(Update, ui_example_system.run_if(in_state(SettingState::Open)))
            .add_systems(Update, update_settings_state)
            ;
    }
}

pub fn update_settings_state(keyboard_input: Res<ButtonInput<KeyCode>>, state: Res<State<SettingState>>, mut next_state: ResMut<NextState<SettingState>>) {
